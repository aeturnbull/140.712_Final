...
##### The three main questions I want to answer are: 
    1. Are there patterns of textual features within the simulation transcripts that cluster together? 
    ...
    3. Are there correlations between either MD characteristics or actor rating and predicted probability the colleagues think the physician disclosed prognosis?
...
sum_measures %>%
    arrange(uid) %>%
    mutate(disclose = dta$death) %>%
    select(phys_verb_prop, phys_noun_prop, phys_adj_prop, phys_adv_prop, disclose) %>%
    ggpairs(aes(color=factor(disclose), alpha=0.5))
```
